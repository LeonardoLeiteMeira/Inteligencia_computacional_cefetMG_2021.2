...
def get_derivative_of_X_line_1(y, w1, w2, y1, y2, sigma1):
    return (y - yd)*w2*((y1-y2)/(w1+w2)**2)*w1*((x-x1_line)/(sigma1**2))

def get_derivative_of_X_line_2(y, w1, w2, y1, y2, sigma2):
    return (y - yd)*w1*((y2-y1)/(w1+w2)**2)*w2*((x-x2_line)/(sigma2**2))

def get_derivative_of_Sigma_1(y, w1, w2, y1, y2, sigma1):
    return (y - yd)*w2*((y1-y2)/(w1+w2)**2)*w1*((x-x1_line)**2/(sigma1**3))

def get_derivative_of_Sigma_2(y, w1, w2, y1, y2, sigma2):
...
def gradient_step(y, y_goal, w1, w2, y1, y2):
    ...
    x1_line =   x1_line -  alpha * get_derivative_of_X_line_1(y, w1, w2, y1, y2, sigma1)
    x2_line = x2_line - alpha * get_derivative_of_X_line_2(y, w1, w2, y1, y2, sigma2)

    sigma1 = sigma1 - alpha * get_derivative_of_Sigma_1(y, w1, w2, y1, y2, sigma1)
    sigma2 = sigma2 - alpha * get_derivative_of_Sigma_2(y, w1, w2, y1, y2, sigma2)
...
for i in range (0,1000):
    y_array = [get_Y(x, p1, p2, q1, q2, x1_line, x2_line, sigma1, sigma2) for x in x_array]

    for j in range(adjusts):
        random_index = randint(0, x_array.length - 1)
    ...
    error = np.mean([get_error(y_array[i], goal_y_list[i]) for i in range(len(y_array))])
